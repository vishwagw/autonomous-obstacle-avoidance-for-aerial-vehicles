# modified detector model
# revised test verision with resized output frames:
import cv2
import numpy as np
import argparse
import os
...
def obstacle_detection(video_path):
    ...
    ret, frame = cap.read()
    if not ret:
        print("Error: Can't receive frame from video")
        return
    ...
    zone_width = int(original_width * zone_width_percent)
    left_boundary = int((original_width - zone_width) / 2)
    right_boundary = int((original_width + zone_width) / 2)
    ...
    while True:
        ...
        cv2.line(vis_frame, (left_boundary, 0), (left_boundary, original_height), (0, 255, 0), 2)
        cv2.line(vis_frame, (right_boundary, 0), (right_boundary, original_height), (0, 255, 0), 2)
        ...
        for contour in contours:
            ...
            relative_distance = 1 - (y / original_height)
            obstacle_distance = f"~{int(relative_distance * 100)}% away"
            
            # Estimate size
            relative_size = (w * h) / (original_width * original_height)
        ...
        cv2.putText(vis_frame, f"Frame: {frame_number}/{total_frames}", (original_width - 200, original_height - 20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Resize display frames to 640x480
        vis_frame_resized = cv2.resize(vis_frame, (display_width, display_height))
        fg_mask_resized = cv2.resize(fg_mask, (display_width, display_height))
        
        # Show frames
        cv2.imshow('Obstacle Detection', vis_frame_resized)
        cv2.imshow('Detection Mask', fg_mask_resized)
        
        # Control playback speed and allow pausing
        key = cv2.waitKey(30)  # Delay in milliseconds (adjust for faster/slower playback)
        if key == ord('q'):  # Press 'q' to quit
            break
        elif key == ord(' '):  # Press spacebar to pause/resume
            cv2.waitKey(0)  # Wait indefinitely until a key is pressed
        elif key == ord('s'):  # Press 's' to step through frame by frame
            cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Obstacle detection from video file')
    ...
    obstacle_detection(args.video_path)
